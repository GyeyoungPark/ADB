...
public:
  ...
  virtual int classify(Packet *p) {
    ...
    } else if (balanceAlg_ == 4) {
	    /*
	    //my own balancing Algorithm
	    */
	unsigned hash = Hash(fid, 0, seed_);
	double value = size;
	packet_arrive++;
	double tmp_time = ts-btime;
	if(tmp_time>time_interval){				//reset counter
		packet_arrive=1;
		btime = ts;

	} else {
		double traffic_load = packet_arrive/tmp_time;
		if(traffic_load > traffic_threshold){		//heavy traffic

		} else {					//not heavy traffic
			if(query(hash) > flowsize_threshold){	//large flow
	...
	sketch = new ADSketch(interval_, hash_len_, length_); //hash_len=4, length=6401 default
	sketch->init(hash, now);
	value = sketch->Query_count(hash);





    } else if(balanceAlg_ == 5) {
		/*
		 *Random Packet Spraying
		 * */
	    unsigned hash = Hash(ts, 1, 1, seed_);	//pseudo random


      return lookup(hash, h->prio());
    }
  }
...
private:
  ...
  static unsigned int packet_arrive=0;
